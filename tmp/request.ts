...
export const buildValsFromKeys: BuildValsFromKeys = (keys) =>
  keys.map((_, index) => `$${index + 1}`);

export const checkIso8601Format = (date: string): boolean =>
  iso8601Regex.test(date);

export const getParameter: GetParameter = (event, key, required) => {
  let value: Value = null;
  if (event.queryStringParameters !== null)
    value = event?.queryStringParameters[key] ?? null;
...
export const getRequestBodyKeys: GetRequestBodyKeys = (event) =>
  Object.keys(JSON.parse(event.body));
...
export const getRequestBodyValues: GetRequestBodyValues = (event) =>
  Object.values(JSON.parse(event.body));
